...
uploaded_utterance_file = st.file_uploader("Upload the Utterance CSV or xlsx file", accept_multiple_files=False)
uploaded_routes_file = st.file_uploader("Upload the Routes txt file", accept_multiple_files=False)


if uploaded_utterance_file is not None:
    
    try:
        bytes_data = uploaded_utterance_file.getvalue()
        stringio = StringIO(uploaded_utterance_file.getvalue().decode("utf-8"))
